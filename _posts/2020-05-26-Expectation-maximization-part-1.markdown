...
Expectation Maximisation (EM) is a flexible framework to estimate the parameters of 
such models. It is an alternative to direct optimisation using Maximum likelihood estimation (MLE),
which typically works off-the-shelf for single distribution models. MLE is a widely used method that 
asserts that the best estimates are those that explain the evidence - the sample observations - 
...
accounting for the heterogeneity of the underlying populations, even if there is no attribute 
that indicates group membership - two examples will follow.

Single distributions try to fit many diverse profiles into a one-size-fits-all model. It is like
trying to fit one shoe size to different feet. In contrast, mixture models allow to try and fit 
different sizes so long as the shoe model remains the same.
...
tool. For any other application, generative models may be more suitable, for example in
...
two random variables (rv). Each observation $x$ comes from a sample from the latent rv followed by a sample from 
the corresponding mixture rv. The goal is to estimate the parameters of the joint probability
distribution.

The mainstream approach to machine learning has a different language than the probabilistic approach, and this can
throw people off - it certainly did and still does to me. However looking at model inference as a 
probability estimation exercise will pay off as it helps answer questions and shed light on overlooked aspects.
