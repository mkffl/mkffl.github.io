...
Given a recognizer that outputs scores, we are enclined to ask if the tool can differentiate between ${\omega_0}$ and ${\omega_1}$ instances. We can first look at the distribution of scores by class and visually inspect if ${\omega_1}$ instances have higher scores. With good separation, there should be two distinct distributions that don't overlap too much, and perfect separation should have no overlap.
...
[![Class Conditional Distributions](/assets/demo11.png){: width="1200px"}](https://i.imgur.com/9MndDQS.png)
{: refdef}
...
- Imagine a classifiation task using a univariate feature set. A randomly drawn dataset would have two columns, one for the feature, call it s, and one for the variable to predict, call it y. The dependent field y values are either w1 (target) or w0 (non-target). We need to decide on a rule, i.e. a way to partition s for new instances.
- Let's look at the conditional distribution p(s|w_0) and p(s|w_1), estimated by binning s and computing the frequencie of the non-target and target instances and Suppose that we get the following estimates.
[Insert code]
[Insert Demo11]
- Further assume equal prior probabilities for w0 and w1, p(w0) = p(w_1) = 0.5 and that we want to minimise the number of misclassified instances. What rule should we choose? 
- It makes sense to label a new instance as w_1 if p(s|w_0) > p(s|w_1) and w_0 otherwise. We'll choose w1 if s > c because the chances of misclassifying the instance are lower for the w1 choice. If s < c we'll pick w_0. 
- With equal priors, the likellihood of s determines the probability of w_i. Minimising the likeihood of error is equivalent to maximising the likelihood of the right choice. Note that in this example there is only one cut-off point, but in general there could be multiple partitions / cut-offs. 
- That's the Bayes decision criterion, which also applies in more general contexts. If we expect more target instances than non-targets, i.e. p(w1) > p(w_0), Then the criterion becomes "Choose w_1" if p(w_0)P(s|w_0)/p(s) < p(w_1)P(s|w_1)/p(s) i.e. p(s|w_1)/p(s|w_0) > p(w_0)/p(w_1). If the cost associated with missing targets is higher than that of missing non-targets, also called a false alarm, then the criterion becomes "Choose w_1" if p(w_0)P(s|w_0)Cfa < p(w_1)P(s|w_1)Cmiss i.e. lr(w_1) > p(w_0)*Cfa/p(w_1)*Cmiss
